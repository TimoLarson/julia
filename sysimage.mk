...
$(build_private_libdir)/compiler_and_base.so: $(build_private_libdir)/compiler_and_base.ji $(build_private_libdir)/compiler_and_base.ji.bc
	$(build_private_libdir)/../../../usr/tools/llvm-dis $(build_private_libdir)/compiler_and_base.ji.bc
	$(build_private_libdir)/../../../usr/tools/clang -g -shared -fpic $(build_private_libdir)/compiler_and_base.ji.bc -o $(build_private_libdir)/compiler_and_base.so
...
$(build_private_libdir)/libcorecompiler.ji: $(COMPILER_SRCS)
	@$(call PRINT_JULIA, cd $(JULIAHOME)/base && \
	$(call spawn, $(JULIA_EXECUTABLE)) -C "$(JULIA_CPU_TARGET)" --output-ji $(call cygpath_w,$@).tmp2 \
		--output-incremental=yes --topbase=yes --startup-file=no -g0 -O0 compiler/compiler.jl)
	@mv $@.tmp2 $@
...
$(build_private_libdir)/syslib.ji: $(COMPILER_SRCS)
	@$(call PRINT_JULIA, cd $(JULIAHOME)/base && \
	$(call spawn, gdb --args $(JULIA_EXECUTABLE)) -C "$(JULIA_CPU_TARGET)" --output-ji $(call cygpath_w,$@).tmp2 \
		--output-incremental=yes --topbase=yes --startup-file=no -g0 -O0 syslib.jl $(RELBUILDROOT))
		#--startup-file=no -g0 -O0 syslib.jl)
	@mv $@.tmp2 $@

$(build_private_libdir)/syslib.so: $(build_private_libdir)/syslib.ji.bc
	$(build_private_libdir)/../../../usr/tools/clang -shared -fpic $(build_private_libdir)/syslib.ji.bc -o $(build_private_libdir)/syslib.so
...
$(build_private_libdir)/sys2.ji: $(build_private_libdir)/corecompiler2.ji $(JULIAHOME)/VERSION $(BASE_SRCS) $(STDLIB_SRCS)
	@$(call PRINT_JULIA, cd $(JULIAHOME)/base && \
	if ! JULIA_BINDIR=$(call cygpath_w,$(build_bindir)) $(call spawn, gdb --args $(JULIA_EXECUTABLE)) -g1 -O0 -C "$(JULIA_CPU_TARGET)" --output-ji $(call cygpath_w,$@).tmp $(JULIA_SYSIMG_BUILD_FLAGS) \
			--topbase=yes --startup-file=no --warn-overwrite=yes sysimg.jl $(RELBUILDROOT); then \
		echo '*** This error might be fixed by running `make clean`. If the error persists$(COMMA) try `make cleanall`. ***'; \
		false; \
	fi )
	@mv $@.tmp $@
