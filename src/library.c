...
JL_DLLEXPORT void jl_lib_restore_system_image_data(const char *buf, size_t len);
...
static void jl_load_sysimg_so(void)
{
    ...
    jl_lib_restore_system_image_data(sysimg_data, *plen);
}
...
static void jl_write_module(jl_serializer_state *s, uintptr_t item, jl_module_t *m)
{
    ...
    for (i = 1; i < m->bindings.size; i += 2) {
        if (table[i] != HT_NOTFOUND) {
            ...
            record_gvar(s, jl_get_llvm_gv(lib_native_functions, (jl_value_t*)b),
...
jl_value_t *jl_lib_find_ptr = NULL;
static void jl_write_values(jl_serializer_state *s)
{
    ...
    for (i = 0, len = backref_table_numel * 2; i < len; i += 2) {
        ...
        record_gvar(s, jl_get_llvm_gv(lib_native_functions, v), ((uintptr_t)DataRef << RELOC_TAG_OFFSET) + reloc_offset);
                ...
                if (m->invoke == jl_fptr_const_return) {
                    ...
                    if (jl_is_method(m->def->def.method)) {
                        ...
                        if (builtin_id) { // found in the table of builtins
                            ...
                            jl_get_function_id(lib_native_functions, m, &invokeptr_id, &specfptr_id); // see if we generated code for it
                            if (invokeptr_id) {
                                if (invokeptr_id == -1) {
...
static void jl_write_gv_syms(jl_serializer_state *s, jl_sym_t *v)
{
    ...
    int32_t gv = jl_get_llvm_gv(lib_native_functions, (jl_value_t*)v);
    if (gv != 0) {
...
static void jl_prune_type_cache(jl_svec_t *cache)
{
    ...
    for (i = 0; i < l; i++) {
        ...
        if (ti == NULL)
            break;
        if (ptrhash_get(&backref_table, ti) != HT_NOTFOUND || jl_get_llvm_gv(lib_native_functions, ti) != 0)
            ...
            if (singleton && (ptrhash_get(&backref_table, singleton) != HT_NOTFOUND ||
                        jl_get_llvm_gv(lib_native_functions, singleton) != 0))
                jl_svecset(cache, ins++, ti);
        }
    }
    if (i > ins) {
...
static void jl_collect_missing_backedges_to_mod(jl_methtable_t *mt)
{
    ...
    if (backedges) {
        ...
        for (i = 1; i < l; i += 2) {
            ...
            jl_array_t **edges = (jl_array_t**)ptrhash_bp(&lib_edges_map, (void*)caller);
...
static void collect_backedges(jl_method_instance_t *callee) JL_GC_DISABLED
{
    ...
    if (backedges) {
        ...
        for (i = 0; i < l; i++) {
            jl_method_instance_t *caller = (jl_method_instance_t*)jl_array_ptr_ref(backedges, i);
            jl_array_t **edges = (jl_array_t**)ptrhash_bp(&lib_edges_map, caller);
            if (*edges == HT_NOTFOUND)
                *edges = jl_alloc_vec_any(0);
...
{
    jl_array_t **pcallees = (jl_array_t**)ptrhash_bp(&lib_edges_map, (void*)caller),
                *callees = *pcallees;
...
static void jl_collect_backedges(jl_array_t *s, jl_array_t *t)
{
    ...
    size_t i;
    void **table = lib_edges_map.table;
    for (i = 0; i < lib_edges_map.size; i += 2) {
...
JL_DLLEXPORT int jl_lib_save_incremental(const char *fname, jl_array_t *worklist)
{
    JL_TIMING(SAVE_MODULE);
    printf("jl_lib_save_incremental() fname: %s\n", fname);
    ...
    // Create the Julia metadata
    {
        ios_t *s = NULL;
        s = jl_lib_create_system_image(native_code);

        // Create an archive containing these method instances
        jl_lib_dump_native(native_code, tmpaname, (const char*)s->buf, (size_t)s->size);
    }
...
JL_DLLEXPORT ios_t *jl_lib_create_system_image(void *_native_data)
{
    ios_t *f = (ios_t*)malloc_s(sizeof(ios_t));
    ios_mem(f, 0);
    lib_native_functions = _native_data;
    jl_save_system_image_to_stream(f);
    return f;
}

JL_DLLEXPORT size_t ios_write_direct(ios_t *dest, ios_t *src);
JL_DLLEXPORT void jl_lib_save_system_image(const char *fname)
{
    ios_t f;
    if (ios_file(&f, fname, 1, 1, 1, 1) == NULL) {
...
JL_DLLEXPORT void jl_lib_set_sysimg_so(void *handle);

// Takes in a path of the form "usr/lib/julia/sys.so" (jl_lib_restore_system_image should be passed the same string)
JL_DLLEXPORT void jl_lib_preload_sysimg_so(const char *fname)
{
    if (jl_sysimg_handle)
        return; // embedded target already called jl_lib_set_sysimg_so
    ...
    if (!is_ji) { // .ji extension => load .ji file only
        jl_lib_set_sysimg_so(jl_load_dynamic_library(fname, JL_RTLD_GLOBAL | JL_RTLD_NOW, 1));
...
JL_DLLEXPORT void jl_lib_restore_system_image(const char *fname)
{
#ifndef JL_NDEBUG
