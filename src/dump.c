...
static void jl_serialize_value_(jl_serializer_state *s, jl_value_t *v, int as_literal) JL_GC_DISABLED
{
    ...
    if (jl_is_svec(v)) {
        ...
        if (codeinst->natived) {
            jl_serialize_value_cstring(s, (char*)codeinst->functionObject);
            jl_serialize_value_cstring(s, (char*)codeinst->specFunctionObject);
        }
...
static jl_value_t *jl_deserialize_value_code_instance(jl_serializer_state *s, jl_value_t **loc) JL_GC_DISABLED
{
    ...
    if (natived) {
        // Record numbered function names and put this code instance on a list to be linked later
        codeinst->functionObject = jl_deserialize_value_cstring(s);
        codeinst->specFunctionObject = jl_deserialize_value_cstring(s);
...
static void jl_link_shared_lib(const char *libpath)
{
    ...
    while (i < natived_list.len) {
        ...
        jl_dlsym(lib, codeinst->functionObject, (void**)&(codeinst->invoke), 1);
        jl_dlsym(lib, codeinst->specFunctionObject, (void**)&(codeinst->specptr), 1);
        i += 1;
    }
}
